module Shared.TextUtils (boundedSubString, scoreEnglishFrequency,
  scoreEnglishHist, englishHist, repeatToLength, subString, trimWhitespace,
...
scoreEnglishFrequency testText = letterE + whiteSpace + nonPrintable
  where hist = charHistogram testText
        frequencyOf = letterFrequency hist
        textLength = fromIntegral $ length testText
        nonPrintable = 0.0 - (sum(drop 127 hist) / textLength)
...
charHistCombine hist fromChar toChar = histCombine hist (fromEnum fromChar) (fromEnum toChar)

charListsHistCombine :: HistArray -> String -> String -> HistArray
